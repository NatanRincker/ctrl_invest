...
async function getUserAssetPositionsSummary(user_id) {
  const result = await database.query({
    ...
    SELECT
      ...
      p.yield, p.realized_pnl, a.yfinance_compatible, a.asset_type_code
...
function computeAssetPosition(position, transaction) {
  if (
    ...
    console.log("------ position.avg_cost ------");
    console.log(position.avg_cost);
    console.log("positionAvgCost");
    console.log(positionAvgCost);
    ...
    let updatedQuantity = new Decimal(position.quantity);
    let updatedTotalCost = new Decimal(position.total_cost);
